...
IngredientsEncas = db.Table(
    "IngredientsEncas",
    db.Column(
        "encas_id",
        db.Integer,
        db.ForeignKey("Encas.id", ondelete="CASCADE", onupdate="CASCADE"),
        primary_key=True,
    ),
    db.Column(
        "ingredient_id",
...
class Encas(db.Model):
    __tablename__ = "Encas"
    id = db.Column(db.Integer, primary_key=True) 
    nom_famille_encas = db.Column(db.String( 255), nullable=False)
    ...
    ingredients = db.relationship(
        "Ciqual", secondary=IngredientsEncas, lazy="subquery"

    )

    @classmethod
    def find_by_id(cls, _id) -> "Encas":
        return cls.query.filter_by(id=_id).first()

    @classmethod
    def find_all(cls, page, per_page) -> List["Encas"]:
